...
@message_router.post("/", response={201: MessageOut}, auth=JWTAuth())
def create_message(request, message_in: MessageIn):
    message = Message.objects.create(**message_in.dict())
    return 201, message

@message_router.get("/", response=list[MessageOut], auth=JWTAuth())
...
@message_router.put("/{message_id}", response={200: MessageOut}, auth=JWTAuth())
...
@message_router.delete("/{message_id}", response={204: None}, auth=JWTAuth())
