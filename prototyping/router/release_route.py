...
@release_router.post("/", response={201: ReleaseOut}, auth=[QueryTokenAuth(), HeaderTokenAuth()])
def create_release(request, release_in: ReleaseIn):
    file_obj = request.FILES.get('file')
    release = Release.objects.create(**release_in.dict(), file=file_obj)
    return release

@release_router.get("/", response=List[ReleaseOut], auth=[QueryTokenAuth(), HeaderTokenAuth()])
...
@release_router.put("/{release_id}", response=ReleaseOut, auth=[QueryTokenAuth(), HeaderTokenAuth()])
...
@release_router.delete("/{release_id}", response={204: None}, auth=[QueryTokenAuth(), HeaderTokenAuth()])
